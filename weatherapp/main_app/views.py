...
def index(request):
  ...
  if request.method == "POST":
    ...
    weather_data1, daily_forecasts1 = fetch_weather_and_forcast(city1, APIKEY, current_weather_url, forecast_url)

    if city2:
      weather_data2, daily_forecasts2 = fetch_weather_and_forcast(city2, APIKEY, current_weather_url, forecast_url)
    else: 
...
def fetch_weather_and_forcast(city,api_key, current_weather_url,forecast_url):
  ...
  lat,lon = response['coords']['lat'], response['coord']['lon']
